...
function examples.splitCursors(pattern)
	mc.action(function(ctx)
		...
		local function pushCursor(cursor, startCol, endCol)
			...
			local col = vs[2]
			newCursor:setVisual({ pos[1], col + startCol }, { pos[1], col + endCol })
		end
...
function examples.alignCursors()
	mc.action(function(ctx)
		...
		local prevLine = nil
		ctx:forEachCursor(function(cursor)
			local col = #lines[cursor:line() - startLine + 1] > 0 and cursor:col() or 0
			-- if col == 0 then
			--     cursor:delete()
			--     return
			-- end
...
function examples.handleMouse()
	mc.action(function(ctx)
		...
		if existingCursor then
			...
			local mainCursor = ctx:mainCursor()
			mainCursor:clone()
			mainCursor:setPos(mouseDragPos):setVisualAnchor(mouseDragPos)
		end
	end)
end
...
function examples.handleMouseDrag()
	mc.action(function(ctx)
		if mouseDragPos == nil then
			mouseDragPos = ctx:numCursors() == 1 and ctx:mainCursor():getPos() or getMousePos()
			mouseDragAdd = true
		end
...
function examples.insertVisual()
	...
	mc.action(function(ctx)
		...
		ctx:forEachCursor(function(cursor)
			cursor:feedkeys(
				(cursor:atVisualStart() and "" or "o") .. "<esc>" .. (mode == TERM_CODES.CTRL_V and "" or "^"),
				{ keycodes = true }
			)
		end)
	end)
...
local function matchAddCursor(direction, add)
	mc.action(function(ctx)
		...
		local c = vim.api.nvim_buf_get_text(0, row - 1, col, row - 1, col + 1, {})
		local searchWord = c[1]:match("[a-zA-Z]") ~= nil and true or false
		if not mainCursor:hasSelection() then
			...
			if cursorChar ~= "" and isKeyword(cursorChar) and string.find(cursorWord, cursorChar, 1, true) then
				searchWord = true
...
local function searchAddCursor(direction, add)
	...
	mc.action(function(ctx)
		...
		if mainCursor:hasSelection() then
			mainCursor:feedkeys((mainCursor:atVisualStart() and "" or "o") .. TERM_CODES.ESC)
		end
		addCursor(ctx, function(cursor)
			cursor:perform(function()
				vim.fn.search(regex, (direction == -1 and "b" or ""))
...
local function regexAddAllCursors(ctx, regex)
	...
	while true do
		...
		elseif firstMatchPos[1] == newPos[1] and firstMatchPos[2] == newPos[2] then
			break
		end
	end
...
--- @param direction? -1 | 1
--- @param add boolean
local function documentHighlightAddCursor(direction, add)
	mc.action(function(ctx)
		if direction == 1 then
			local pos = require("illuminate.goto").get_next_reference(true)
			addCursor(ctx, function(cursor)
				cursor:setPos({ pos[1], pos[2] + 1 })
			end, { addCursor = add })
		else
			local pos = require("illuminate.goto").get_prev_reference(true)
			addCursor(ctx, function(cursor)
				cursor:setPos({ pos[1], pos[2] + 1 })
			end, { addCursor = add })
		end
...
function examples.documentHighlightMatchCursors()
	local function getRange(mode)
		local s = vim.api.nvim_buf_get_mark(0, "[")
		local e = vim.api.nvim_buf_get_mark(0, "]")
		if mode == "char" then
	...
	local references = require("illuminate.reference").buf_get_references(vim.api.nvim_get_current_buf())
	setOpfunc(function(vmode)
		mc.action(function(ctx)
			local mainCursor = ctx:mainCursor()
			-- Update main cursor, use correct mark range.
			for _, ref in ipairs(references) do
				-- diagnostic is 0-based line and col
				if posInRange(ref[1][1] + 1, ref[1][2]+1, getRange(vmode)) then
					local newCursor = mainCursor:clone()
					newCursor:setPos({ ref[1][1] + 1, ref[1][2]+1 })
					newCursor:setMode("n")
				end
			end
			mainCursor:delete()
...
--- @param direction? -1 | 1
--- @param opts? vim.diagnostic.GotoOpts
function examples.documentHighlightAddCursor(direction, opts)
	documentHighlightAddCursor(direction, true, opts)
end

--- @param direction? -1 | 1
--- @param opts? vim.documentHighlight.GotoOpts
function examples.documentHighlightSkipCursor(direction, opts)
	documentHighlightAddCursor(direction, false, opts)
end
...
function examples.operator(opts)
	...
	setOpfunc(function(opMode)
		...
		if vMode ~= nil then
			...
			state.pattern =
				string.format(state.wordBoundary and "\\M\\<%s\\>" or "\\M%s", getSelection(getRange(true), vMode)[1])
		elseif state.pattern == "" then
			-- If user doesn't provide a pattern, we get pattern specified by '[
			-- and '] marker.
			state.pattern =
				string.format(state.wordBoundary and "\\<%s\\>" or "%s", getSelection(getRange(false), opMode)[1])
		end
		...
		setOpfunc(function(mode)
			...
			matchCursorsRange(state.pattern, range, getSelection(range, mode), state.visual)
		end)
		vim.schedule(function()
			vim.api.nvim_feedkeys(string.format("g@"), "ni", false)
		end)
